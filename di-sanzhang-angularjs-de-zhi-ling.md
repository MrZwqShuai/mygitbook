...
> 用了=标识符之后，可以清楚看到当我们改变指令的属性和值的时候会在组件内外直接映射，这是一种双向绑定，这里使用等号的双向绑定类似手动设置$watch方法

##### &回调表达式前缀标识符

> &符号表示属性设置的表达式会被当做作用域中的一个函数，当属行被调用的时候该表达式就会执行，这里的changeMyName就是元素中change-my-name属性

有了这些基础我们就可以开始自定义自己的指令了 。
